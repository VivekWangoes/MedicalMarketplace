...
class PatientMedicalProfileView(APIView):
    ...
    def put(self, request):
        # try:
            print(request.user.patient_profile.patient_medical_profile,request.data)
            ...
            if request.data.get('alergies_data', ''):
                ...
                if request.data.get('alergies_data')['new']:
                    alergies_obj = Alergies.objects.filter(name=request.data.get('alergies_data')['new'])
                    ids.append(alergies_obj[0].id)
                    if not alergies_obj:
            ...
            medication_obj = Medication.objects.filter(id__in=request.data.get('medication_data', None))
            patient_obj.medication.set(medication_obj)
            dieseas_obj = Dieseas.objects.filter(id__in=request.data.get('dieseas_data',''))
            patient_obj.cronic_dieseas.set(dieseas_obj)
            injuries_obj = Injuries.objects.filter(id__in=request.data.get('injuries_data',''))
            patient_obj.injuries.set(injuries_obj)
            surgery_obj = Surgery.objects.filter(id=request.data.get('surgery_data',''))
